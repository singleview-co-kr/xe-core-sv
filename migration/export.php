<?php 
     ...
     **/
        ...
        while($document_info = $oMigration->fetch($document_result)) {
			// $obj = null;
			$obj->document_srl = null;
			$obj->mid = null;
			$obj->category = null;
			$obj->title = null;
			$obj->content = null;
			$obj->readed_count = null;
			$obj->voted_count = null;
			$obj->user_id = null;
			$obj->nick_name = null;
			$obj->email = null;
			$obj->homepage = null;
			...
			$obj->ipaddress = null;
			$obj->status = null;
			$obj->comment_status = null;
			//$obj->allow_comment = null;
			//$obj->lock_comment = null;
			$obj->is_notice = null;
			//$obj->is_secret = null;  // XE2에 없는 컬럼
			$obj->regdate = null;
            ...
            if($document_info->category_srl) 
				$obj->category = $category_list[$document_info->category_srl]->title;
            $obj->title = htmlentities($document_info->title, ENT_XML1, "UTF-8" );
            $obj->content = htmlentities($document_info->content, ENT_XML1, "UTF-8" );
            ...
            //$obj->is_secret = $document_info->is_secret;  // XE2에 없는 컬럼
            ...
            while($comment_info = $oMigration->fetch($comment_result)) {
                ...
                $comment_obj->content = htmlentities($comment_info->content, ENT_XML1, "UTF-8" );
            ...
            while($file_info = $oMigration->fetch($file_result)) {
				...
				$file_obj = new stdClass();  // $file_obj = null;
                $file_obj->filename = $filename;
                $file_obj->file = $file;
                $file_obj->download_count = $download_count;
                $files[] = $file_obj;
