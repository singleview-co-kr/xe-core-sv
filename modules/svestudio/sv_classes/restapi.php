...
//			die ("Couldn't initialize a cURL handle");
			return new BaseObject(-1, 'failed to initialize a cURL handle');
		}
...
//			die ("No HTTP code was returned");
			return new BaseObject(-1, 'No HTTP code was returned');
		}
		else 
		{
...
//			die("failed to request");
			return new BaseObject(-1, 'failed to request. error :'.$error);
		}
		//echo "Transaction-ID : " . $response["transactionId"] . "\n";
		//return $response["json"];
		if( $response["json"] )
		{
			if( (int)($response["json"]['status'] / 200) == 1 || $response["json"]['status'] == 'NONE' || 
				$response["json"]['status'] == 'BUILT') //succeed || no latest data || complete built report
				$oOutput = new BaseObject();
			else
				$oOutput = new BaseObject(-1, 'NVR server returned error');
			
			$oOutput->add('transactionId', $response["transactionId"]);
			$oOutput->adds($response["json"]);

			return $oOutput;
		}
		else
			return new BaseObject('succeed');
	}
/**
 * @brief
 */
...
//        echo "Transaction-ID : " . $response["transactionId"] . "\n";
		...
		//if( $response["json"] )
		//{
			if( (int)($response["json"]['status'] / 200) == 1 ) // succeed
				$oOutput = new BaseObject('succeed');
			else
			{
				$oOutput = new BaseObject(-1, 'NVR server returned error');
				$oOutput->add('transactionId', $response["transactionId"]);
 ...
 */
	public function DOWNLOAD($url, $localpath)
	{
		...
		curl_close($ch);
		fclose($fp);
		if (empty ($code)) // 정상 처리되면 $code == 200
		{
//			die ("No HTTP code was returned");
			return new BaseObject(-1, 'No HTTP code was returned');
		}
